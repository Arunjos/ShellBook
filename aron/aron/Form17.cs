    ...
    public partial class Form17 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (comboBox1.Text.Length > 0)
            {
                ...
                ad = new SQLiteDataAdapter("SELECT * FROM addexdetails WHERE    date >='" + a + "' AND date <= '" + b + "'AND name = '" + comboBox1.Text + "'", cnn);
        ...
        public void refresh()
        {
            ...
            {
                cnn.Open();
                ad = new SQLiteDataAdapter("SELECT * FROM addexdetails WHERE    date >='" + a + "' AND date <= '" + b + "'AND name = '" + nam + "'", cnn);
                SQLiteCommandBuilder sqlCommandBuilder = new SQLiteCommandBuilder(ad);
                dataTable = new DataTable();
                ad.Fill(dataTable);
                BindingSource bindingSource = new BindingSource();
